## 첫번째 시도는 하드코딩으로 값을 나오게 하였다면
## 두번째 시도는 __init__ 값을 Getter, Setter를 통해 넘겨 받는것으로 코딩
...
class Bicycle(Vehicle):
    ...
    def __str__(self):
        return "이 자전거는 현재 {idxSpeed}km/h로 주행 중입니다.".format(idxSpeed = self._speed)
...
class NormalCar(Vehicle):
    ...
    def __str__(self):
        return "이 일반 자동차는 현재 {idxSpeed}km/h로 주행 중입니다.".format(idxSpeed=self._speed)
...
class SportsCar(Vehicle):
    ...
    def __str__(self):
        return "이 스포츠카는 현재 {idxSpeed}km/h로 주행 중입니다.".format(idxSpeed=self._speed)
