...
class Clock:
    ...
    def tick(self):
        ...
        """
        # 코드를 쓰세요
        Counter.limit = Clock.SECONDS
        self.second = Counter.tick(self.second)

        if True == Counter.tick():
            Counter.limit = Clock.MINUTES
            self.minute = Counter.tick(self.minute)

        if True == Counter.tick():
            Counter.limit = Clock.HOURS
            self.hour = Clock.tick(self.hour)
